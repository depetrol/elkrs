...
use elk_alg_common::compaction::one_dimensional_compactor::OneDimensionalCompactor;
use elk_alg_common::compaction::{CNodeId, CNodeOrigin};
use elk_alg_common::networksimplex::{NGraph, NNodeId, NetworkSimplex};
use elk_core::options::PortSide;

use crate::graph::{LGraphArena, LNodeId, LPortId};
...
pub trait NsSpacings {
    fn horizontal_spacing(&self, cg: &elk_alg_common::compaction::CGraph, n1: CNodeId, n2: CNodeId)
        -> f64;
    fn is_vertical_segments_of_same_edge(
        &self,
        cg: &elk_alg_common::compaction::CGraph,
        n1: CNodeId,
        n2: CNodeId,
    ) -> bool;
}
...
fn add_edge_constraints(
    ...
    let mut l_edge_cnodes: Vec<(crate::graph::LEdgeId, CNodeId)> = Vec::new();
    for c in 0..count {
        match odc.cgraph.cnodes[c].origin {
