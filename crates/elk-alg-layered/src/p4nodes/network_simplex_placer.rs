...
//! (reusing `elk_alg_common::networksimplex`). Includes the `NodeFlexibility`
...
use elk_alg_common::networksimplex::{NEdgeId, NGraph, NNodeId, NetworkSimplex};
use elk_core::options::{NodeLabelPlacement, PortConstraints, PortSide};
use elk_graph::math::Spacing;
use elk_graph::properties::EnumSet;

use crate::graph::{LEdgeId, LGraphArena, LGraphId, LNodeId, LPortId, NodeType};
        ...
        for &n in &a.layer(layer).nodes {
            for sp in a.node_port_side_view(n, PortSide::SOUTH) {
                let other: Option<LNodeId> =
                    a.port(sp).properties.try_get(&crate::internal_properties::PORT_DUMMY);
