...
use crate::options_gen::NodePlacementStrategy;
use elk_graph::graph::ElkGraph;
...
pub fn apply_layout(
    ...
    elk: &mut ElkGraph,
    lgraph: LGraphId,
    elk_parent: elk_graph::graph::NodeId,
) -> Result<(), String> {
...
fn apply_node_layout(
    ...
    elk: &mut ElkGraph,
    lnode: LNodeId,
    elknode: elk_graph::graph::NodeId,
    offset: KVector,
) {
    // position/layer ids
    let node_id: i32 = a
...
fn port_label_placement_is_fixed(
    ...
    !placement.contains(elk_core::options::PortLabelPlacement::INSIDE)
...
fn apply_edge_layout(
    ...
    elk_core::elkutil::apply_vector_chain(elk, &bend_points, section);
...
fn port_absolute_anchor(a: &LGraphArena, port: crate::graph::LPortId) -> KVector {
    let p = a.port(port);
