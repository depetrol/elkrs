...
pub fn prepare_graph_for_layout(
    ...
    crate::p3order::processor_configuration(p3, a, graph, &mut config)?;
    crate::p4nodes::processor_configuration(p4, a, graph, &mut config)?;
    crate::p5edges::processor_configuration(p5, a, graph, &mut config)?;
...
fn configure_graph_properties(a: &mut LGraphArena, graph: LGraphId) -> Result<(), String> {
    ...
    // Spacings are computed on demand in Rust (see crate::spacings); no
