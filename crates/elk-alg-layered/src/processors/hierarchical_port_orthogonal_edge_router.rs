...
use elk_core::options::{PortConstraints, PortSide, SizeConstraint};
...
use crate::p5edges::direction::RoutingDirection;
use crate::p5edges::orthogonal_routing_generator::OrthogonalRoutingGenerator;
...
fn correct_slanted_layer(a: &mut LGraphArena, layer: crate::graph::LayerId) {
    let nodes = a.layer(layer).nodes.clone();
    for node in nodes {
        if a.node(node).node_type != NodeType::EXTERNAL_PORT {
            continue;
