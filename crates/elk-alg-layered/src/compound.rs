...
use elk_core::options::{Direction, PortConstraints, PortLabelPlacement, PortSide};
use elk_graph::math::{KVector, KVectorChain};
use elk_graph::properties::{EnumSet, JavaCloneable, JavaString, PropertyMap};

use crate::graph::{LEdgeId, LGraphArena, LGraphId, LLabelId, LNodeId, LPortId, NodeType};
use crate::internal_properties as iprops;
use crate::lgraph_util::{self, PortPropertyHolder};
...
impl CrossHierarchyEdge {
    ...
    fn actual_source(&self, a: &LGraphArena) -> LPortId {
        ...
        if a.node(node).node_type == NodeType::EXTERNAL_PORT {
            if let Some(crate::internal_properties::Origin::LPort(p)) =
...
impl Preprocessor {
    fn set_sides_of_ports_to_sides_of_dummy_nodes(&self, a: &mut LGraphArena) {
        for (&external_port, &dummy_node) in &self.dummy_node_map {
            a.node(dummy_node)
                .properties
                .set(&iprops::ORIGIN, crate::internal_properties::Origin::LPort(external_port));
                        ...
                        let dummy_node = match self.dummy_node_map.get(&port) {
                            ...
                            None => {
                                ...
                                a.node(d).properties.set(
                                    &iprops::ORIGIN,
                                    crate::internal_properties::Origin::LPort(port),
                                );
            ...
            if !labels.is_empty() {
                ...
                for curr_label in labels {
                    ...
                    let target_index: i64 = match placement {
                        ...
                        elk_core::options::EdgeLabelPlacement::CENTER => {
                            get_shallowest_edge_segment(&edge_segments)
                        }
                        elk_core::options::EdgeLabelPlacement::TAIL => 0,
                        _ => -1,
                    };
    ...
    fn ensure_self_loop_dummy(
        ...
        a.node(d)
            .properties
            .set(&iprops::ORIGIN, crate::internal_properties::Origin::LPort(port));
        self.dummy_node_map.insert(port, d);
        a.graph_mut(nested_graph).layerless_nodes.push(d);
        d
    }
    ...
    fn introduce_hierarchical_edge_segment(
        ...
        if default_external_port.is_none() || !merge_external_ports || parent_end_port.is_some() {
            ...
            let dummy_origin_port = match a.node(dummy_node).properties.try_get(&iprops::ORIGIN) {
                Some(crate::internal_properties::Origin::LPort(p)) => p,
    ...
    fn create_external_port_dummy(
        ...
        if a.port(outside_port).node == Some(parent_node) {
            if let Some(&d) = self.dummy_node_map.get(&outside_port) {
                ...
                a.node(d).properties.set(
                    &iprops::ORIGIN,
                    crate::internal_properties::Origin::LPort(outside_port),
                );
            ...
            a.node(d)
                .properties
                .set(&iprops::ORIGIN, crate::internal_properties::Origin::LPort(dummy_port));
