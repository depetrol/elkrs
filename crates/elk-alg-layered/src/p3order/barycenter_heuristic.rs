...
use crate::graph::{LGraphArena, LNodeId, NodeType};
...
fn calculate_barycenter(
    ...
    for free_port in a.node(node).ports.clone() {
        ...
        let fixed_ports: Vec<crate::graph::LPortId> = if forward {
...
fn port_type_for(direction: bool) -> crate::options_gen::PortType {
    if direction {
        crate::options_gen::PortType::OUTPUT
    } else {
        crate::options_gen::PortType::INPUT
    }
}
