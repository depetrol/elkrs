...
use elk_core::providers::fixed::all_outgoing_edges;
...
pub fn import_graph(g: &ElkGraph, kgraph: NodeId) -> Result<(FArena, FGraph), String> {
    ...
    let mut elem_map: HashMap<NodeId, crate::graph::FNodeId> = HashMap::new();
...
fn transform_nodes(
    ...
    arena: &mut FArena,
    fgraph: &mut FGraph,
    elem_map: &mut HashMap<NodeId, crate::graph::FNodeId>,
) {
    let mut index = 0;
...
fn transform_edges(
    ...
    arena: &mut FArena,
    fgraph: &mut FGraph,
    elem_map: &HashMap<NodeId, crate::graph::FNodeId>,
) -> Result<(), String> {
    for &knode in &g.node(parent_node).children {
