...
use elk_graph::graph::{EdgeId, ElkGraph, LabelId, NodeId, PortId};
use elk_graph::math::{KVector, KVectorChain};

use crate::graph::{DCDirection, DCElement, DCExtension, DCGraph};
use crate::options;
...
impl ElkGraphTransformer {
    ...
    pub fn apply_layout(&mut self, g: &mut ElkGraph, graph: &DCGraph) {
        ...
        for &(key, elem) in &self.element_mapping {
            ...
            match key {
                ElementKey::Edge(edge) => {
                    ...
                    g.edge(edge)
                        .properties
                        .set(&elk_core::options::JUNCTION_POINTS, jps);
                }
        ...
        for &(edge, dir) in &self.incoming_extensions_mapping {
            ...
            elk_core::elkutil::apply_vector_chain(g, &new_points, section);
