...
use elk_graph::math::{ElkPadding, KVector};
...
impl LayoutProvider for RectPackingLayoutProvider {
    fn layout(&mut self, g: &mut ElkGraph, layout_node: NodeId) -> Result<(), String> {
        ...
        if !stackable {
            ...
            let mut box_provider = elk_core::providers::box_layouter::BoxLayoutProvider;
        ...
        elk_core::elkutil::translate_aligned(
            g,
            layout_node,
...
fn min_size_pre_processor(g: &mut ElkGraph, graph: NodeId) {
    ...
    if let Some(parent) = g.node(graph).parent {
        ...
        let mut adapter = elk_core::adapters::ElkGraphAdapter::new(g, parent);
        let min_size2 =
            elk_alg_common::nodespacing::process_node_size(&mut adapter, graph, false, true);
