...
use elk_alg_common::triangulation::TEdge;
use elk_graph::graph::{ElkGraph, NodeId};
use elk_graph::math::{ElkRectangle, KVector};

use crate::graph::Graph;
use crate::options::{self, RootSelection, SpanningTreeCostFunction};
...
impl ElkGraphImporter {
    pub fn import_graph(g: &mut ElkGraph, input_graph: NodeId) -> Result<(Self, Graph), String> {
        // calculate margins
        {
            let mut adapter = elk_core::adapters::ElkGraphAdapter::new(g, input_graph);
        ...
        for elk_node in children {
            ...
            while importer.node_map.contains_key(&kv_bits(vertex)) {
                let r = random.get_or_insert_with(|| elk_core::javacompat::JavaRandom::new(1));
                vertex.add_xy(
            ...
            let margin: elk_graph::math::ElkMargin =
                g.node(elk_node).properties.get(&elk_core::options::MARGINS);
    ...
    pub fn cost(&self, graph: &Graph, e: &TEdge) -> f64 {
        match self.cost_function_id {
            ...
            SpanningTreeCostFunction::INVERTED_OVERLAP => {
                ...
                s.sub(r1.center());
                let s = s.length();
                -(elk_alg_common::utils::overlap(r1, r2) - 1.0) * s
            }
        }
    }
    ...
    pub fn apply_positions(&self, g: &mut ElkGraph, graph: &Graph) {
        ...
        let padding: elk_graph::math::ElkPadding =
            g.node(self.elk_graph).properties.get(&options::PADDING);
        elk_core::elkutil::resize_node(
            g,
            self.elk_graph,
        ...
        for e in &contained {
            let _ = g
                .edge(*e)
                .properties
                .get(&elk_core::options::JUNCTION_POINTS);
        }
        elk_core::elkutil::translate(
            g,
            self.elk_graph,
            -min_x + padding.left,
            -min_y + padding.top,
