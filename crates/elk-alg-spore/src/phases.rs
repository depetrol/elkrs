...
use elk_alg_common::spore::depth_first_compact;
use elk_alg_common::tree::Forest;
use elk_alg_common::triangulation::{bowyer_watson_triangulate, naive_min_st, TEdge};
use elk_graph::math::KVector;

use crate::graph::Graph;
use crate::options::TreeConstructionStrategy;
...
fn grow_at(
    tree: &Forest<usize>,
    r: usize,
    nodes: &mut [elk_alg_common::spore::Node],
    ...
    for &c in &tree.nodes[r].children {
        ...
        let t = elk_alg_common::utils::overlap(&nodes[r_node].rect, &nodes[c_node].rect);
        if t > 1.0 {
