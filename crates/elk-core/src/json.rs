...
use elk_graph::graph::{ElementId, ElkGraph, EdgeId, NodeId, PortId, SectionId, ShapeId};
use elk_graph::properties::{PropertyHolder, PropertyMap};
...
impl<'r> JsonExporter<'r> {
    ...
    fn transform_label(&mut self, g: &ElkGraph, label: elk_graph::graph::LabelId) -> Value {
        let mut obj = Map::new();
        let l = g.label(label);
