...
impl<'g> AdapterGraph for ElkGraphAdapter<'g> {
    ...
    fn node_padding(&self, n: NodeId) -> Spacing {
        self.elk
            .node(n)
            .properties
            .get(&crate::options::PADDING)
    }
    fn set_node_padding(&mut self, n: NodeId, padding: Spacing) {
        self.elk
            .node(n)
            .properties
            .set(&crate::options::PADDING, padding);
    }
    fn node_margin(&self, n: NodeId) -> Spacing {
        self.elk.node(n).properties.get(&crate::options::MARGINS)
    }
    fn set_node_margin(&mut self, n: NodeId, margin: Spacing) {
        self.elk.node(n).properties.set(&crate::options::MARGINS, margin);
    }

    fn port_side(&self, p: PortId) -> PortSide {
        self.elk.port(p).properties.get(&crate::options::PORT_SIDE)
    }
    ...
    fn port_margin(&self, p: PortId) -> Spacing {
        self.elk.port(p).properties.get(&crate::options::MARGINS)
    }
    fn set_port_margin(&mut self, p: PortId, margin: Spacing) {
        self.elk.port(p).properties.set(&crate::options::MARGINS, margin);
    }
...
fn default_port_comparator(elk: &ElkGraph, p1: PortId, p2: PortId) -> std::cmp::Ordering {
    ...
    let side2: PortSide = elk.port(p2).properties.get(&crate::options::PORT_SIDE);
    ...
    let index1 = elk.port(p1).properties.try_get(&crate::options::PORT_INDEX);
    let index2 = elk.port(p2).properties.try_get(&crate::options::PORT_INDEX);
