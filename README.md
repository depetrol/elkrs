...
Cargo workspace under `crates/`, each mirroring an ELK plugin:

| Crate | Mirrors |
|---|---|
| `elk-graph` | `org.eclipse.elk.graph` — model, properties, KVector math, JSON I/O |
| `elk-core` | `org.eclipse.elk.core` — options, engine, fixed/box/random layouters |
| `elk-alg-common` | `org.eclipse.elk.alg.common` — node sizing, compaction, spore/triangulation |
| `elk-alg-layered` | `org.eclipse.elk.alg.layered` — the main effort (~62k Java lines) |
| `elk-alg-force` | force + stress |
| `elk-alg-radial` · `elk-alg-mrtree` · `elk-alg-rectpacking` · `elk-alg-spore` · `elk-alg-disco` · `elk-alg-topdownpacking` | the remaining algorithms |
| `elk-cli` | mirrors the oracle CLI; reads ELK JSON on stdin, prints laid-out JSON |

Supporting trees: `oracle/` (Java CLI on the 0.11.0 jars, generates goldens),
`tools/elk-sources/` (downloaded 0.11.0 source jars — the authoritative porting
reference; the vendored `elk/` tree is 0.12.0-SNAPSHOT and read-only), `goldens/`
(input cases + expected oracle output), `tools/` (fuzzer + comparators).
...
cargo test --workspace                         # all 205 tests
cargo test -p elk-cli --test goldens           # the 201-case byte-exact golden suite
```
...
2. **Replicated JUnit tests** (`crates/*/tests/junit_*.rs`): 118 of ELK's own
   `@Test`s rewritten as Rust tests — black-box ones run through
   `elk_cli::create_elk()`, white-box ones call the ported APIs directly
